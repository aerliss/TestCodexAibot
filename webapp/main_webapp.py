...
async def _resolve_tg_id(request: Request) -> int:
    ...
    if auth:
        tg_id = signer.verify(auth)
        if tg_id is not None:
            session_token = secrets.token_urlsafe(24)
            await redis.set(f"session:{session_token}", tg_id, ex=cfg.session_ttl_seconds)
            return tg_id

    raise HTTPException(status_code=401, detail="No valid session token")


async def get_current_user(request: Request) -> User:
    tg_id = await _resolve_tg_id(request)
